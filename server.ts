...
import { connectRedis, disconnectRedis } from "./src/infra/redis.js";
import { closeQueues } from "./src/infra/queues.js";
import { eventBus } from "./src/infra/eventBus.js";
import { telegramEngine, initTelegramBots } from "./src/agents/telegramEngine.js";
import { facebookEngine } from "./src/agents/facebookEngine.js";
import { routeIncoming, injectPool } from "./src/hub/messageRouter.js";
import { injectWorkerDeps, initAIWorker, closeAIWorker } from "./src/ai/worker.js";
import { injectPool as injectTriggerPool, startPolling as startTriggerPolling, stopPolling as stopTriggerPolling, handleEvent } from "./src/automation/triggerEngine.js";
import { recordMetric, getHourlyMetric, getDailyMetric } from "./src/analytics/collector.js";
import { injectDeps as injectAutomationDeps, initAutomationWorker, closeAutomationWorker } from "./src/automation/actionExecutor.js";
import { createCalendarEvent } from "./src/services/calendar.js";
import { PACKAGE_CATALOG } from "./src/configs/package-catalog.js";
const { Pool } = pkg;
...
async function startServer() {
    ...
    helmet({
      contentSecurityPolicy: IS_PROD ? {
        directives: {
          ...
          connectSrc: ["'self'", "https://api.twilio.com", "https://generativelanguage.googleapis.com"],
          frameAncestors: ["'none'"],
        },
      } : false,
  ...
  // JSON body limit: 2MB global — endpoints que necesitan más (OCR, uploads) lo
  // sobreescriben individualmente para evitar DoS por payloads enormes.
  ...
  // Limpieza periódica del Map de rate limiting para evitar memory leak.
  // Cada 5 minutos eliminamos entradas cuya ventana ya expiró.
  setInterval(() => {
    const now = Date.now();
    for (const [key, entry] of rateCounts) {
      if (now > entry.reset) rateCounts.delete(key);
    }
  }, 5 * 60_000);
  ...
  // Límite extendido SOLO para endpoints que manejan imágenes/documentos grandes
  app.use('/api/ocr', express.json({ limit: '15mb' }));
  app.use('/api/expediente', express.json({ limit: '20mb' }));

  // ── CSRF: validar Origin en requests mutantes (POST/PATCH/DELETE) ──────────
  const allowedOrigins = [process.env.APP_URL].filter(Boolean) as string[];
  app.use('/api', (req: any, res: any, next: any) => {
    if (['POST', 'PATCH', 'PUT', 'DELETE'].includes(req.method)) {
      const origin = req.get('origin');
      // Permitir requests sin Origin (Postman, curl, server-to-server) y localhost en dev
  ...
  const aiGenerate = async (
    ...
    if (openaiClient) {
      ...
      if (isText && !visionMode) {
        try {
          console.warn("Usando OpenAI GPT-4o-mini como respaldo final (texto).");
          const chat = await openaiClient.chat.completions.create({
            model: "gpt-4o-mini",
    ...
    // SEGURIDAD: el salt legacy solo se usa para migrar hashes SHA-256 → bcrypt.
    // En producción DEBE estar definido vía env var.
    const SALT = (() => {
      const s = process.env.PASSWORD_SALT;
      if (!s && IS_PROD) {
        console.error('[FATAL] PASSWORD_SALT no definido en producción.');
        process.exit(1);
      }
      return s || 'hdreams_dev_salt_2026';
    })();
    ...
    type BotCandidate = {
      ...
      stage: 'interesado' | 'agendo' | 'confirmocita' | 'confirmodd' | 'bienvenida' | 'no_show';
  ...
  const refreshCookieOpts = {
    ...
    const revokedRefreshHashes = new Map<string, number>(); // hash → timestamp de expiración
    // Limpieza periódica — elimina solo tokens cuyo TTL ya pasó (7 días).
    // A diferencia de clear(), esto NO reactiva tokens revocados prematuramente.
    setInterval(() => {
      const now = Date.now();
      for (const [hash, expiresAt] of revokedRefreshHashes) {
        if (now > expiresAt) revokedRefreshHashes.delete(hash);
      }
    }, 3600_000); // cada hora
    ...
    const requireRole = (...allowed: string[]) =>
      (req: any, res: any, next: any) => {
        ...
        // SEGURIDAD: comparación exacta de roles — .includes() permitía bypass parcial
        // (ej. un rol "super_administracion_temp" pasaría la verificación de "administracion").
        const normalised = payload.role.toLowerCase();
        if (allowed.length && !allowed.some(r => normalised === r.toLowerCase()))
    ...
    app.post("/api/auth/refresh", async (req, res) => {
      try {
        ...
        if (isDbConnected) {
          ...
          revokedRefreshHashes.set(oldHash, Date.now() + 7 * 24 * 3600_000);
        }
      ...
      try {
        ...
        if (refreshToken) {
          ...
          if (isDbConnected) {
            ...
            revokedRefreshHashes.set(tokenHash, Date.now() + 7 * 24 * 3600_000);
          }
        }
...
Si la información no es suficiente para responder, indícalo.`;
    ...
    app.post("/api/ocr", async (req, res) => {
      try {
        ...
        if (!image || !docType) return res.status(400).json({ error: 'Faltan parámetros' });

        const base64Data = image.replace(/^data:image\/\w+;base64,/, '');
        let rawText = '';

        try {
          rawText = await cloudVisionOCR(base64Data, 'image/jpeg');
        } catch (err: any) {
          console.warn("[OCR] Cloud Vision fallback to LLM vision:", err.message);
          const prompt = `Extrae los datos de este documento oficial (${docType}). Retorna SOLO un JSON válido sin Markdown con los siguientes campos clave (si no están, pon ""): nombres, apellidoPaterno, apellidoMaterno, curp, calle, numeroExterior, numeroInterior, colonia, ciudad, delegacion, codigoPostal, folioIne, municipio, estado.`;
          const text = await aiGenerate([prompt, { inlineData: { data: base64Data, mimeType: 'image/jpeg' } }], { visionMode: true });
          const jsonStr = text.replace(/```json/g, '').replace(/```/g, '').trim();
          return res.json(JSON.parse(jsonStr));
        }

        const prompt = `Extrae los datos del siguiente texto de un documento (${docType}). Retorna SOLO un JSON válido sin Markdown con los siguientes campos (si no están, pon ""): nombres, apellidoPaterno, apellidoMaterno, curp, calle, numeroExterior, numeroInterior, colonia, ciudad, delegacion, codigoPostal, folioIne, municipio, estado.\n\nTEXTO:\n${rawText}`;
  ...
  "delegacion": "alcaldía, delegación o estado"
    ...
    const expedienteStaticAuth = (req: any, res: any, next: any) => {
        ...
        if (modulo === 'clientes') {
          ...
          records.forEach((r: any, i: number) => {
            ...
            mockDb.botCandidates.push({
              ...
              lastStatusChange: r.lastStatusChange || new Date().toISOString(),
            });
            imported++;
          });
      ...
      app.post("/api/recruitment/candidates", (req, res) => {
        ...
        if (!c.lastStatusChange) c.lastStatusChange = new Date().toISOString();
        if (!mockDb.botCandidates) mockDb.botCandidates = [];
...
FASES DE SEGUIMIENTO:
  - INTERESADO: Etapa inicial tras primer contacto exitoso.
  - AGENDO: El candidato aceptó la cita y tiene fecha/hora programada.
  - CONFIRMO CITA: Confirmación explícita previa a la asistencia.
  - CONFIRMO DE D,DO: Validación de documentos y perfil avanzado.
  - BIENVENIDA: Contratación final exitosa.
  - NO SHOW: El candidato no asistió a su cita agendada.
...
Responde SOLO con JSON exacto (sin markdown, sin bloques de código):
{
  ...
  "razon_humano": null
        ...
        try {
          ...
          if (!candidate) {
            candidate = {
              ...
              profile: "pendiente", stage: "interesado", lastStatusChange: new Date().toISOString(),
              assignedAgent: (mockDb.botCandidates.length % 5) + 1,
          ...
          if (aiData.extractedData) {
            ...
            if (d.stage && candidate.stage !== d.stage) {
              candidate.stage = d.stage;
              candidate.lastStatusChange = new Date().toISOString();
            }
      ...
      // ═══════════════════════════════════════════════════════════════════════════
      //  PROCESO DIARIO DE ACTUALIZACIÓN (RECLUTAMIENTO)
      // ═══════════════════════════════════════════════════════════════════════════
      /**
       * Ejecuta la limpieza diaria de candidatos:
       * - Si tenían cita ayer o antes y no pasaron a 'BIENVENIDA', mover a 'NO SHOW'.
       */
      const runDailyRecruitmentCleanup = async () => {
        console.log("[Recruitment] Iniciando limpieza diaria de candidatos...");
        const now = new Date();
        const todayStr = now.toISOString().split('T')[0];
        let noShowCount = 0;
        let recontactCount = 0;

        if (mockDb.botCandidates) {
          for (const c of mockDb.botCandidates) {
            // 1. Marcar como No Show si pasó la fecha
            if (['agendo', 'confirmocita'].includes(c.stage) && c.appointmentDate && c.appointmentDate < todayStr) {
              c.stage = 'no_show';
              c.lastStatusChange = now.toISOString();
              c.notes = (c.notes || '') + `\n[Auto] Movido a No Show por inasistencia el ${c.appointmentDate}.`;
              noShowCount++;
            }

            // 2. Re-contactar si lleva 3 días en No Show
            if (c.stage === 'no_show' && c.lastStatusChange) {
              const daysInNoShow = Math.floor((now.getTime() - new Date(c.lastStatusChange).getTime()) / (24 * 3600_000));
              if (daysInNoShow === 3) {
                recontactCount++;
                const msg = `Hola ${c.name || 'candidato'}, vimos que no pudiste asistir a tu entrevista en Heavenly Dreams. ¡Aun tenemos vacantes disponibles! ¿Te gustaria reagendar para esta semana? Responde REAGENDAR para darte horarios.`;

                // Intentar enviar mensaje (vía el motor de WhatsApp si está configurado)
                try {
                  // Obtenemos el ID del agente asignado para enviar desde su número
                  const agentId = `AG-RECLUTAMIENTO-${String(c.assignedAgent || 1).padStart(3, '0')}`;
                  await whatsappEngine.sendText(agentId, c.phone, msg);
                  c.messages.push({ role: 'bot', text: msg, ts: now.toISOString() });
                  c.notes = (c.notes || '') + `\n[Auto] Mensaje de re-contacto enviado tras 3 días de No Show.`;
                } catch (err) {
                  console.warn(`[Recruitment] No se pudo enviar re-contacto a ${c.phone}:`, (err as Error).message);
                }
              }
            }
          }
        ...
        if (noShowCount > 0 || recontactCount > 0) {
          saveMockDb(mockDb);
          pushNotif('Actualización CRM Reclutamiento',
            `${noShowCount} inasistencias detectadas. ${recontactCount} mensajes de re-contacto enviados.`,
            'info', 'admin');
        }
        console.log(`[Recruitment] Proceso completado. No Shows: ${noShowCount}, Re-contactos: ${recontactCount}.`);
      };

      // Programar para ejecutarse cada 24 horas (u 1 AM para evitar picos)
      setInterval(runDailyRecruitmentCleanup, 24 * 3600_000);
      // Ejecutar una vez al inicio para poner al día el sistema
      setTimeout(runDailyRecruitmentCleanup, 15_000);
...
Responde EXCLUSIVAMENTE con un JSON: {"category":"...","confianza":0.0-1.0,"razonamiento":"..."}`;
      ...
      //  ADMIN / EXPENSES
      // ═══════════════════════════════════════════════════════════════════════════
      app.get('/api/admin/expenses', requireRole('gerente', 'admin'), (req, res) => {
        res.json(mockDb.expenses || []);
      });

      app.post('/api/admin/expenses', requireRole('gerente', 'admin'), async (req, res) => {
        if (!mockDb.expenses) mockDb.expenses = [];
        const exp = { ...req.body, id: 'EXP-' + Date.now() };
        mockDb.expenses.unshift(exp);
        saveMockDb(mockDb);
        res.json(exp);
        // Integración automática con Google Calendar
        if (exp.tipo === 'recurrente' || exp.tipo === 'unico') {
          await createCalendarEvent(exp);
        }
      });

      app.patch('/api/admin/expenses/:id', requireRole('gerente', 'admin'), (req, res) => {
        const list = mockDb.expenses || [];
      ...
      app.delete('/api/admin/expenses/:id', requireRole('gerente', 'admin'), (req, res) => {
        if (mockDb.expenses) {
          mockDb.expenses = mockDb.expenses.filter((e: any) => e.id !== req.params.id);
          saveMockDb(mockDb);
        }
        res.json({ ok: true });
      });
      ...
      app.get('/api/admin/packages', (req, res) => {
        res.json((mockDb as any).packages || PACKAGE_CATALOG);
      });
      app.post('/api/admin/packages', requireRole('gerente', 'admin'), (req, res) => {
        (mockDb as any).packages = req.body;
        saveMockDb(mockDb);
        res.json({ ok: true });
      });
  ...
  process.on('SIGINT',  () => shutdown('SIGINT'));
      ...
      // Requisito para despliegues en Docker / Railway donde Node sirve todo
      if (IS_PROD) {
        // Si se corre desde `dist/server.cjs`, la ruta build/ está un nivel arriba o al lado.
        // Dockerfile lo copia a `/app/build`, así que asumiendo process.cwd() === '/app'
